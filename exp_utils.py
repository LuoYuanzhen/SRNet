...
def draw_hidden_heat_compare_img(filename, nndatas, srnndatas, title=""):
    ...
    nn_samples = [torch.vstack([nndatas[i][j] for j in idxs]) for i in range(n_h)]
    srnn_samples = [torch.vstack([srnndatas[i][j] for j in idxs]) for i in range(n_h)]
...
def encode_individual_from_json(json_file, elite_name):
    ...
    net_params = NetParameters(neurons=neurons,
